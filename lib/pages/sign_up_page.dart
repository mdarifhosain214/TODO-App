...
class _SignUpPageState extends State<SignUpPage> {
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              textItem(labelText: 'Password', controller: _passwordController,obscureText: true),
              const SizedBox(
                height: 30,
              ),
              colorButton(),
  ...
  Widget colorButton() {
    return InkWell(
      onTap: ()async {
        setState((){circuler =true;});
       try{
         firebase_auth.UserCredential userCredential=await _auth.createUserWithEmailAndPassword(
             email: _emailController.text,
             password: _passwordController.text);
         setState((){circuler =false;});
         const snackBar =   SnackBar(content: Text("Create account successfully",style: TextStyle(fontSize: 18),));
         ScaffoldMessenger.of(context).showSnackBar(snackBar);
         Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context)=>HomePage()), (route) => false);

       }catch(e){
         final snackBar = SnackBar(content: Text(e.toString(),style: const TextStyle(fontSize: 18)));
         ScaffoldMessenger.of(context).showSnackBar(snackBar);
         setState((){circuler =false;});
  ...
  Widget buttonItem(
      ...
      required double buttonSize,required Function() onTap}) {
    return InkWell(
      onTap: onTap,
      child: Container(
        height: 60,
